...
[
    <item:supplementaries:cog_block> : "Example for a block that was removed but now has text put comma after me",
...
<item:supplementaries:firefly_jar> :"Light source. Obtained by clicking a firefly with an empty normal jar.
you can't release fireflies from them.",
...
<item:supplementaries:sack> :"An early game shulker box. You can only carry a maximum of two. Carry 3 or more and you will be slowed by their weight.
Sacks will react to gravity. To keep one from falling you'll have to place a supporting block either under it or above it.",
<item:supplementaries:blackboard> :"You'll be able to draw on it in black and white.
It will keep what's drawn on it once broken and you'll be able to duplicate your masterpieces just like you can with banners. Craft them with a water bucket to clear them in bulk. They can also be edited by holding a dye or a sponge to clear it all",
...
<item:supplementaries:doormat> :"A hybrid between a sign and a carpet. Cats love to sit on it from time to time.
You can also hide one item under a carpet by shift clicking on its side",
...
<item:supplementaries:clock_block> :"This block reads the hour and minute within your Minecraft world.
It interacts with observers and comparators.",
...
<item:supplementaries:faucet> :"Right-click the facet to turn it on, it will keep dropping items from the inventory of a storage block up to two blocks behind it. You can use it to pour liquids to and from jars, as well as other fluid tanks. A faucet connected to a liquid source will be able to convert a concrete block below it.
able to pour liquids on pancakes (honey, syrup and chocolate) and tipped spikes (lingering pot)",
...
<item:supplementaries:pancake> :"A food item that can be placed down and stacked up to 8 times. 
If you put honey on top of them they will give you a small speed boost effect once you eat them.
They can also accept chocolate from Create (works with faucets and dispensers)",
